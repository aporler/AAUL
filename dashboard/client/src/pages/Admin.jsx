 ...
 * Admin Settings Page
 * 
 * This page is the operator view for dashboard host configuration.
 * It combines network settings stored in the database with server-level
 * settings stored on disk, which is why the page has a little more state than
 * the average form.
 */
  ...
  return (
    <div className="card space-y-4">
            ...
            {config.http.enabled && (
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div>
                  ...
                  />
                  <p className="mt-1 text-xs text-slate-500">Vite dev only — not used in production</p>
                </div>
              </div>
            )}
          </div>
      ...
      <ConfirmDialog
        ...
        title={t("admin.restartServer") || "Restart Server"}
        description={
          t("admin.confirmRestart") || "Restart the server? The dashboard will be unavailable briefly."
        }
        confirmLabel={t("admin.restartServer") || "Restart Server"}
        tone="danger"
...
export default function Admin() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setInternetBaseUrl(settings.internetBaseUrl || "");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card space-y-4">
        ...
        <div>
          <label className="text-xs uppercase tracking-[0.3em] text-slate-400">
            {t("admin.internetBaseUrl")}
          </label>
          <input
            className="input mt-2"
            type="url"
            placeholder={t("admin.internetBaseUrlPlaceholder")}
            value={internetBaseUrl}
            onChange={(e) => setInternetBaseUrl(e.target.value)}
          />
          <p className="mt-2 text-xs text-slate-500">{t("admin.internetBaseUrlHint")}</p>
        </div>
      </div>
