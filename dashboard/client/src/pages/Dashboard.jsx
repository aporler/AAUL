 ...
 * Main fleet view.
 *
 * This page is intentionally stateful because it acts as the operator's control
 * surface: it shows the fleet, subscribes to live updates, opens install and
 * schedule dialogs, and queues actions against agents.
 */
...
// Use background polling only when the live event stream is not connected.
...
function getOsIcon(osInfo) {
  if (!osInfo?.name) return null;
  const n = osInfo.name.toLowerCase();
  if (n.includes("windows")) return <WindowsIcon className="w-4 h-4 text-blue-400" />;
  if (n.includes("darwin") || n.includes("macos") || n.includes("mac os")) return <AppleIcon className="w-4 h-4 text-slate-300" />;
  return <LinuxIcon className="w-4 h-4 text-orange-400" />;
}
...
export default function Dashboard() {
  ...
  // Initial snapshot used before SSE updates start flowing.
  useEffect(() => {
    loadAgents();
  }, [loadAgents]);

  // Keep the table in sync without full page refreshes.
  useEffect(() => {
  ...
  // Some deployments may proxy SSE poorly; polling keeps the page usable there too.
  useEffect(() => {
  ...
  const runCommand = async (path, confirmConfig = null) => {
    const execute = async () => {
  ...
  const updateAllOutdated = async () => {
    const outdatedCount = agents.filter((a) => a.isOutdated).length;
    if (outdatedCount === 0) {
      setNoticeDialog({
        title: t("dashboard.updateOutdated"),
        description: t("dashboard.updateOutdatedNone")
      });
      return;
    }
    setConfirmDialog({
      title: t("dashboard.updateOutdated"),
      description: t("dashboard.confirm.updateOutdated"),
      onConfirm: async () => {
        setConfirmDialog(null);
        await apiFetch("/api/agents/update-outdated", { method: "POST" });
        await loadAgents();
      }
    });
  };
  ...
  const copyInstallCommand = async () => {
    const command = installInfo?.installCommands?.[installPlatform] || installInfo?.installCommand;
    if (!command) {
      return;
    }
  ...
  const isAgentOnline = (lastSeenAt) => {
    if (!lastSeenAt) {
      return false;
    }
    const last = new Date(lastSeenAt).getTime();
    return !Number.isNaN(last) && Date.now() - last <= ONLINE_THRESHOLD_SECONDS * 1000;
  };
  ...
  // Header cards and filter badges both read from the same derived counters.
  const stats = useMemo(() => {
    const total = agents.length;
    const online = agents.filter((a) => isAgentOnline(a.lastSeenAt)).length;
    const outdated = agents.filter((a) => a.isOutdated).length;
    const rebootRequired = agents.filter((a) => a.rebootRequired).length;
    return { total, online, outdated, rebootRequired };
  }, [agents]);
  ...
  const filteredAgents = useMemo(() => {
    ...
    return agents.filter((agent) => {
      const matchesSearch =
        ...
        agent.ip?.toLowerCase().includes(term) ||
      ...
      if (fleetFilter === "online") {
        return isAgentOnline(agent.lastSeenAt);
      }
      if (fleetFilter === "offline") {
        return !isAgentOnline(agent.lastSeenAt);
      }
      if (fleetFilter === "outdated") {
        return agent.isOutdated;
      }
      if (fleetFilter === "reboot") {
        return agent.rebootRequired;
      }
      return true;
    });
  }, [agents, fleetFilter, searchTerm]);

  const HostInfoPanel = ({ stats: hostStats }) => (
    <div className="rounded-2xl border border-ink-700 bg-ink-900/60 p-4">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-xl border border-ink-700 bg-ink-900/70 p-4">
          ...
          <div className="mt-2 text-xs text-slate-500">
            Cores: {hostStats?.cpuCores ?? "--"} / Threads: {hostStats?.cpuThreads ?? "--"}
          </div>
          <div className="mt-3">
            <DonutChart
              used={hostStats?.cpu ?? 0}
              total={100}
              usedLabel="Used"
              freeLabel="Available"
              usedValue={`${hostStats?.cpu ?? "--"}%`}
              freeValue={`${hostStats ? 100 - (hostStats.cpu ?? 0) : "--"}%`}
              size={88}
              thickness={10}
              layout="stacked"
            />
        ...
        <div className="rounded-xl border border-ink-700 bg-ink-900/70 p-4">
          ...
          <div className="mt-2 text-xs text-slate-500">
            {hostStats ? `${formatBytes(hostStats.memUsedBytes)} / ${formatBytes(hostStats.memTotalBytes)}` : "--"}
          </div>
          <div className="mt-3">
            <DonutChart
              used={hostStats?.memUsedBytes ?? 0}
              total={hostStats?.memTotalBytes ?? 0}
              usedLabel="Used"
              freeLabel="Available"
              usedValue={formatBytes(hostStats?.memUsedBytes)}
              freeValue={formatBytes((hostStats?.memTotalBytes || 0) - (hostStats?.memUsedBytes || 0))}
              size={88}
              thickness={10}
              layout="stacked"
            />
        ...
        <div className="rounded-xl border border-ink-700 bg-ink-900/70 p-4">
          ...
          <div className="mt-2 text-xl font-semibold text-white">
            {hostStats?.diskFreeGb ?? "--"} GB
          </div>
          <div className="mt-2 text-xs text-slate-500">
            {hostStats?.diskTotalGb ? `of ${hostStats.diskTotalGb} GB` : ""}
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
          <div className="flex items-center gap-3">
            <div>
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                {latestVersion ? (
                  <span className="rounded-full border border-violet-500/40 bg-violet-500/10 px-2 py-0.5 text-xs font-mono text-violet-300">
                    bundle v{latestVersion}
                  </span>
                ) : null}
              </div>
            </div>
            {/* Live indicator */}
            <div
              className="flex items-center gap-1.5 rounded-full border border-ink-600 bg-ink-800 px-2.5 py-1 text-xs"
              title={liveIndicator ? "Real-time updates active" : "Reconnecting..."}
            >
              <span
                className={`h-2 w-2 rounded-full ${liveIndicator ? "bg-emerald-400 animate-pulse" : "bg-slate-500"}`}
              />
              <span className={liveIndicator ? "text-emerald-400" : "text-slate-500"}>
                {liveIndicator ? "Live" : "..."}
              </span>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            {latestVersion ? (
              <button
                className={`btn ${stats.outdated > 0 ? "btn-primary" : "btn-secondary"} flex items-center gap-2`}
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <StatCard label={t("dashboard.stats.total")} value={stats.total} color="blue" />
        <StatCard label={t("dashboard.stats.online")} value={stats.online} color="green" />
        <StatCard label={t("dashboard.stats.outdated")} value={stats.outdated} color="amber" />
        <StatCard label={t("dashboard.stats.rebootRequired")} value={stats.rebootRequired} color="red" />
      </div>
        ...
        <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            {[
              ...
              { id: "online", label: t("dashboard.filters.online") },
              { id: "offline", label: t("dashboard.filters.offline") },
              { id: "outdated", label: t("dashboard.filters.outdated") },
              { id: "reboot", label: t("dashboard.filters.reboot") }
            ].map((filter) => (
              <button
                key={filter.id}
                className={`rounded-full border px-3 py-1.5 text-xs font-medium transition ${
                  fleetFilter === filter.id
                    ? "border-ocean-400 bg-ocean-500/10 text-ocean-300"
                    ...
                    <td className="px-4 py-4 align-top">
                      <div className="flex flex-col gap-1.5">
                        <div className="flex items-center gap-2">
                          ...
                          {agent.osInfo ? (
                            <div className="group/os relative flex-shrink-0">
                              <span className="cursor-default text-base leading-none select-none">
                                {getOsIcon(agent.osInfo)}
                              </span>
                              <div className="pointer-events-none absolute left-0 top-full z-20 mt-1 hidden min-w-max rounded-lg border border-ink-600 bg-ink-800 px-3 py-2 text-xs shadow-lg group-hover/os:block">
                                <div className="font-semibold text-white">{agent.osInfo.name}</div>
                                {agent.osInfo.version ? (
                                  <div className="text-slate-400">{t("details.osVersion")}: {agent.osInfo.version}</div>
                                ) : null}
                                {agent.osInfo.codename ? (
                                  <div className="text-slate-500 italic">{agent.osInfo.codename}</div>
                                ) : null}
                              </div>
                            </div>
                          ) : null}
                          {/* Agent name with ID tooltip */}
                          <div className="group/name relative">
                            <div className="font-semibold text-white">{agent.displayName}</div>
                            <div className="pointer-events-none absolute left-0 top-full z-10 mt-2 hidden w-56 rounded-xl border border-ink-600 bg-ink-800 px-3 py-2 text-xs text-slate-200 group-hover/name:block">
                              ID: <span className="font-mono break-all">{agent.id}</span>
                            </div>
                          </div>
                        </div>
                        {/* Local / Internet badge */}
                        {agent.isInternet ? (
                          <span className="inline-flex w-fit items-center gap-1 rounded-full border border-violet-500/30 bg-violet-500/10 px-1.5 py-0.5 text-[10px] text-violet-300">
                            🌐 internet
                          </span>
                        ) : (
                          <span className="inline-flex w-fit items-center gap-1 rounded-full border border-blue-500/30 bg-blue-500/10 px-1.5 py-0.5 text-[10px] text-blue-300">
                            🏠 local
                          </span>
                        )}
                      </div>
                          ...
                          <div className="absolute right-0 top-full z-20 mt-2 w-52 rounded-xl border border-ink-600 bg-ink-800 p-2 shadow-soft">
                            {agent.lastSeenAt ? (
                              <>
                                  ...
                                  onClick={() =>
                                    (() => {
                                      ...
                                      return runCommand(
                                        ...
                                        {
                                          title: t("dashboard.actions.runNow"),
                                          description: t("dashboard.confirm.runNow")
                                        }
                                      );
                                    })()
                                  }
                                ...
                                <button
                                  className={`w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-ink-700 ${agent.isOutdated ? "text-amber-300" : "text-slate-200"}`}
                                  onClick={() =>
                                    (() => {
                                      setOpenMenuId(null);
                                      return runCommand(
                                        ...
                                        {
                                          title: t("dashboard.actions.updateAgent"),
                                          description: t("dashboard.confirm.updateAgent")
                                        }
                                      );
                                    })()
                                  }
                                    ...
                                    onClick={() =>
                                      (() => {
                                        ...
                                        return runCommand(
                                          ...
                                          {
                                            title: t("dashboard.actions.cancelUninstall"),
                                            description: t("dashboard.confirm.cancelUninstall")
                                          }
                                        );
                                      })()
                                    }
                                ...
                                onClick={() =>
                                  (() => {
                                    ...
                                    return runCommand(
                                      `/api/agents/${agent.id}/commands/cancel`,
                                      {
                                        title: t("dashboard.actions.cancelPending"),
                                        description: t("dashboard.confirm.cancelPending")
                                      }
                                    );
                                  })()
                                }
              ...
              <div>
                ...
                <div className="mt-2 rounded-xl bg-ink-800 p-4 text-sm text-slate-100">
                  <code className="break-all font-mono">
                    {installInfo.installCommands?.[installPlatform] || installInfo.installCommand}
                  </code>
                </div>
                {installInfo.installUrls?.[installPlatform] ? (
                  <p className="mt-2 text-xs text-slate-500">
                    {t("dashboard.installModal.scriptUrl")}:{" "}
                    <code className="break-all font-mono">
                      {installInfo.installUrls[installPlatform]}
                    </code>
                  </p>
